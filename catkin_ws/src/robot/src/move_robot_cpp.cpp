...
public:
    // CONSTRUCTIOR
    MoveRobot(std::string name) :   // In the argument of the constructor name of action is passed
        _as(nh_, name, false),      // Initialization list: _as...action server class is initialized with
                                    // provided parameters
        action_name(name)           
    {   
        as_.registerGoalCallback(boost::bind(&MoveRobot::goalCB, this));
        as_registerPreemptCallback(boost::bind(&MoveRobot::preemptCB, this))
        
        // Define topics to subscribe to
        odom_sub_ = nh_.subscribe("/odom", 1, &MoveRobot::controlCB, this);
        as_.start();
    }
...
protected:
    ...
    // Ros
    ros::NodeHandle nh_;
    actionlib::SimpleActionServer<robot::MoveRobot> as_;
    ...
    void rotate_velocities(double angle, double global_vel[3], double (&loc_vel)[3]) {
        loc_vel[0] = cos(angle)* global_vel[0] - sin(angle)* global_vel[1];
        loc_vel[1] = sin(angle)* global_vel[1] - cos(angle)* global_vel[0];
        loc_vel[2] = global_vel[2];
    }
...
int main(int argc, char** argv) {
    ...
    MoveRobot moverobot(ros::this_node::getName());

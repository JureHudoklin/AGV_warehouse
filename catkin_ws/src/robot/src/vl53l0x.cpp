...
VL53L0X::VL53L0X(void)
    : address(ADDRESS_DEFAULT), io_timeout(0) // no timeout
      ,
      did_timeout(false)
{
}
...
// mode.
bool VL53L0X::init(int bus, bool io_2v8)
{
    this->bus = bus;
    rc_i2c_init(bus, address);
...
void VL53L0X::writeReg(uint8_t reg, uint8_t value)
{
    ...
    last_status = rc_i2c_write_byte(bus, reg, value);
}
...
void VL53L0X::writeReg16Bit(uint8_t reg, uint16_t value)
{
    ...
    // last_status = Wire.endTransmission();
    last_status = rc_i2c_write_word(bus, reg, value);
}
...
void VL53L0X::writeReg32Bit(uint8_t reg, uint32_t value)
{
    ...
    // last_status = Wire.endTransmission();
    last_status = rc_i2c_write_byte(bus, reg, (value >> 24) & 0xFF);
    last_status = rc_i2c_write_byte(bus, reg, (value >> 16) & 0xFF);
    last_status = rc_i2c_write_byte(bus, reg, (value >> 8) & 0xFF);
    last_status = rc_i2c_write_byte(bus, reg, value & 0xFF);
}
...
uint8_t VL53L0X::readReg(uint8_t reg)
{
    ...
    uint8_t value;
    rc_i2c_read_byte(bus, reg, &value);
    return value;
}
...
uint16_t VL53L0X::readReg16Bit(uint8_t reg)
{
    ...
    uint16_t value;
    rc_i2c_read_word(bus, reg, &value);
    return value;
}
...
uint32_t VL53L0X::readReg32Bit(uint8_t reg)
{
    ...
    uint32_t value;
    uint8_t values[4];
    rc_i2c_read_bytes(bus, reg, 4, values);
...
void VL53L0X::writeMulti(uint8_t reg, uint8_t const *src, uint8_t count)
{
    ...
    last_status = rc_i2c_write_bytes(bus, reg, count, (uint8_t *)src);
}
...
void VL53L0X::readMulti(uint8_t reg, uint8_t *dst, uint8_t count)
{
    ...
    rc_i2c_read_bytes(bus, reg, count, dst);
}
...
uint32_t VL53L0X::millis()
{
    return (uint32_t)(rc_nanos_since_boot() / 1000000);
}
